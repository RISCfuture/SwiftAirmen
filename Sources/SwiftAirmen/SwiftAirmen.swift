...
public enum RiggerRating: Comparable, Hashable, CustomStringConvertible, Sendable {
  ...
  public var description: String {
    switch self {
      case .back(let level): return "back (\(level))"
      case .chest(let level): return "chest (\(level))"
      case .seat(let level): return "seat (\(level))"
      case .lap(let level): return "lap (\(level))"
    }
  }

  public static func < (lhs: Self, rhs: Self) -> Bool {
    switch lhs {
      case .back(let lhsLevel):
        guard case .back(let rhsLevel) = rhs else { return false }
        return lhsLevel < rhsLevel
      case .chest(let lhsLevel):
        switch rhs {
          case .back: return false
          case .chest(let rhsLevel): return lhsLevel < rhsLevel
          default: return true
        }
      case .seat(let lhsLevel):
        switch rhs {
          case .back, .chest: return false
          case .seat(let rhsLevel): return lhsLevel < rhsLevel
          default: return true
        }
      case .lap(let lhsLevel):
        guard case .lap(let rhsLevel) = rhs else { return false }
        return lhsLevel < rhsLevel
    }
  }
  ...
  func descriptionForLevel(_ level: RiggerLevel) -> String {
    switch self {
      case .back(let ratingLevel):
        if level == ratingLevel { return "back" }
        return description
      case .chest(let ratingLevel):
        if level == ratingLevel { return "chest" }
        return description
      case .seat(let ratingLevel):
        if level == ratingLevel { return "seat" }
        return description
      case .lap(let ratingLevel):
  ...
  public static func < (lhs: Self, rhs: Self) -> Bool {
    switch lhs {
      ...
      case .instrument(let lhsCat):
        switch rhs {
          case .categoryClass: return false
...
public enum FlightInstructorRating: Comparable, Hashable, CustomStringConvertible, Sendable {
  ...
  public var description: String {
    switch self {
      case .category(let category): return category.description
      case .instrument(let category): return "IFR-\(category.description)"
      case .sport: return "SPL"
    }
  }
  ...
  public static func < (lhs: Self, rhs: Self) -> Bool {
    switch lhs {
      case .category(let lhsCat):
        guard case .category(let rhsCat) = rhs else { return true }
        return lhsCat < rhsCat
      case .instrument(let lhsCat):
        switch rhs {
  ...
  public var description: String {
    switch self {
      ...
      case .flightInstructor(let ratings, _):
        return "CFI (\(ratings.sorted().map(\.description).joined(separator: ", ")))"
      case .authorizedAircraftInstructor: return "AAI"
      case .remotePilot: return "sUAS"
      case .groundInstructor(let ratings):
        return "\(ratings.sorted().map(\.description).joined())GI"
      case .flightEngineer(let ratings):
        return "FE (\(ratings.sorted().map(\.description).joined(separator: ", ")))"
      case .flightEngineerLessee: return "FE (lessee)"
      case .flightEngineerForeign(let ratings):
        return "FE (foreign, \(ratings.sorted().map(\.description).joined(separator: ", ")))"
      case .mechanic(let ratings):
        return "AMT (\(ratings.sorted().map(\.description).joined(separator: "&")))"
...
public struct Address: CustomStringConvertible, Sendable {
  ...
  var isEmpty: Bool {
    street1 == nil && street2 == nil && city == nil && state == nil && zipCode == nil
      && country == nil && region == nil
  }
  ...
  public var description: String {
    return [street1, street2, cityStateZip, country].compactMap(\.self).joined(separator: "\n")
  }
}
...
public enum Medical: CustomStringConvertible, Sendable, Equatable {
   ...
   */
  case basicMed(
    courseDate: DateComponents,
    expirationDate: DateComponents?,
    CMECDate: DateComponents?
  )

  /// A human-readable description of the certificate.
  public var description: String {
    switch self {
      case .FAA(let `class`, _, _): return `class`.description
