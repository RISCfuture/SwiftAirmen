...
struct AirmanMergingTests {
  ...
  func mergePreferringNewer() {
    ...
    let newAirman = Airman(
      ...
      medical: .FAA(
        .first,
        date: DateComponents(year: 2024, month: 1, day: 1),
        expirationDate: DateComponents(year: 2026, month: 1, day: 1)
      ),
      certificates: [
        .pilot(level: .commercial, ratings: [.instrument(.airplane)], centerlineThrustOnly: false)
      ]
    )
  ...
  func combineCertificatesWithoutDuplicates() {
    let airman1 = Airman(
      ...
      certificates: [
        .pilot(
          level: .private,
          ratings: [.categoryClass(.airplaneSingleEngineLand, level: .private)],
          centerlineThrustOnly: false
        ),
        .flightInstructor(
          ratings: [.category(.airplaneSingleEngine)],
          expirationDate: DateComponents(year: 2025, month: 12, day: 31)
        )
      ]
    )
